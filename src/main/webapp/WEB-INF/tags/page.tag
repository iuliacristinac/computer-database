...
<div class="container text-center">
	<ul class="pagination">
		<li><a href="<c:url value="${url}?page=1&size=${pageSize}" />"
			aria-label="Next"> <span aria-hidden="true">&laquo;</span>
		</a></li>
		<li><c:choose>
				<c:when test="${previous}">
					<a
						href="<c:url value="${url}?page=${page - 1}&size=${pageSize}" />"
						aria-label="Next"> <span aria-hidden="true">&lsaquo;</span>
					</a>
				</c:when>
				<c:otherwise>
		...
		<c:forEach var="i" begin="${page}" end="${pageCount}">
			<c:choose>
				...
				<c:otherwise>
					<li><a
						href="<c:url value="${url}?page=${i}&size=${pageSize}" />">${i}</a></li>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		<li><c:choose>
				...
				<c:otherwise>
					<a href="#" aria-label="Next"> <span aria-hidden="true">&rsaquo;</span>
					</a>
				</c:otherwise>
			</c:choose></li>
		<li><a
			href="<c:url value="${url}?page=${totalPages}&size=${pageSize}" />"
